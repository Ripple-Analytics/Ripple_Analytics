...
(defonce app-state (r/atom
  {:user nil
   ...
   :analysis-results []
   :sync-status :idle
   :api-url (or (js/process.env.API_URL) \"http://localhost:3000\")}))
...
(defn unwatch-folder [folder-path]
...
(defn read-file [filepath]
  ...
  (let [ch (chan)]
    (fs.readFile filepath \"utf8\" (fn [err data]
                                    (if err
                                      (do
    ...
    (fs.readFile filepath (fn [err data]
                           (if err
                             (do
                               ...
                               (async/close! ch))
                             (pdf-parse data (fn [err result]
                                            (if err
                                              (do
                                                (js/console.error \"Error parsing PDF:\" err)
                                                (async/close! ch))
                                              (go (>! ch (:text result)))))))))
    ch))
...
(defn analyze-file [filepath content]
...
(defn folder-list []
  ...
  [:div.folder-list
   ...
   (if (empty? (:watched-folders @app-state))
     ...
     [:ul
      (doseq [folder (:watched-folders @app-state)]
        [:li {:key (:path folder)}
         [:span (:path folder)]
...
(defn recent-analyses []
  ...
  [:div.recent-analyses
   ...
   (if (empty? (:analysis-results @app-state))
     [:p \"No analyses yet\"]
     [:ul
      (doseq [result (take 10 (reverse (:analysis-results @app-state)))]
        [:li {:key (:id result)}
         [:div.result-item
          [:span.filename (:source-url result)]
          [:span.score (str \"Score: \" (format \"%.2f\" (:average-score result)))]])])])
...
(listen-from-main \"sync-all\" (fn [data]
                               (go
